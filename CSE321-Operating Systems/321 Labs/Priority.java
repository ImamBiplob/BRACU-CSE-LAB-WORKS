...
public class Priority {
  public static void main(String [] args) {
    ...
    for(int i=0;i<p_num;i++) {
      ...
      System.out.println("Please enter p"+(i+1)+"'s priority");
    ...
    for( int i=0;i<p_num;i++) {
      avgWT=avgWT+wt[i];
      avgTAT=avgTAT+tat[i];
      System.out.println("\nProcess"+p_ID[i]+"--> Arrival Time="+arrivalTime[i]+" Burst Time="+burstTimeSolid[i]+" priority="+priority[i]+" Completion Time="+ ct[i]+" Turnaround Time="+tat[i]+" Waiting Time="+wt[i]);
    }
